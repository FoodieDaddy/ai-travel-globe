...
interface Props {
  place: ChinaPlace | null;
  onMarkVisited: (placeId: string) => void;
  onPhotoUpload: (placeId: string, photoUrl: string) => void;
  onClose: () => void;
  onCenterPlace: (place: ChinaPlace) => void;
}

export const ChinaMemoryCard: React.FC<Props> = ({ 
  place, 
  onMarkVisited, 
  onPhotoUpload,
  onClose,
  ...
  const imageUrl = useMemo(() => {
    ...
    // 经典备用图
    if (place.name === '杭州') return "https://images.unsplash.com/photo-1528164344705-47542687000d?w=500&q=80";
    if (place.name === '苏州') return "https://images.unsplash.com/photo-1599581561565-d05051e5e6e3?w=500&q=80";
    if (place.name === '上海') return "https://images.unsplash.com/photo-1508804185872-d7badad00f7d?w=500&q=80";
    if (place.name === '拉萨') return "https://images.unsplash.com/photo-1548013146-72479768bada?w=500&q=80";
        ...
        <div className="absolute bottom-3.5 left-4 right-4 z-10">
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            <span className="text-[11px] text-white/90 font-medium drop-shadow flex items-center gap-0.5">
              ...
              {place.province} {place.visitedAt ? `· ${place.visitedAt}` : (place.season ? `· ${place.season}` : '')}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="p-4.5 space-y-4 max-h-[280px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
        ...
        {place.visited && place.note && (
          <div className="bg-white/[0.03] border border-white/5 rounded-xl p-3.5 relative overflow-hidden">
            ...
            <span className="text-[9px] font-bold text-white/30 tracking-widest uppercase block mb-1">旅行印记</span>
        ...
        {!place.visited && (
          <div className="flex items-center justify-between text-[10.5px] text-white/50 bg-white/[0.02] border border-white/5 rounded-xl p-3">
            <span className="font-medium">尚未解锁神州足迹</span>
            {place.season && <span className="font-mono text-amber-400/80">建议游玩: {place.season}</span>}
          </div>
        )}
        ...
        {place.legendStory && (
          <div className="space-y-1">
            <span className="text-[9px] font-bold text-white/35 tracking-widest uppercase block">历史故事</span>
        ...
        {place.highlights && place.highlights.length > 0 && (
          <div className="text-[11px] text-white/50 flex flex-wrap gap-x-2 gap-y-1 items-center pt-2.5 border-t border-white/5">
            <span className="text-[9px] font-extrabold text-white/30 uppercase tracking-widest shrink-0">核心景点:</span>
...
export default ChinaMemoryCard;
